...
use rayon::prelude::*;
use wgpu::{
    CompositeAlphaMode, Extent3d, MemoryHints, SurfaceConfiguration, TextureFormat, TextureUsages,
...
impl App {
    ...
    fn new(render_configs: RenderConfigs, window_size: PhysicalSize<u32>, title: String) -> Self {
        Self {
            render_configs,
            render_state: None,
            window_size,
    ...
    fn resize(&mut self, new_size: PhysicalSize<u32>) {
        ...
        rs.back_face_pass
            .resize(&rs.device, rs.size.width, rs.size.height);
        rs.front_face_pass
            .update_model_view_proj_uniform(rs.cube_scaling, &rs.camera, &rs.queue);
        rs.back_face_pass
            .update_model_view_proj_uniform(rs.cube_scaling, &rs.camera, &rs.queue);
        rs.canvas_pass
            .resize(&rs.device, rs.size.width, rs.size.height);
    ...
    fn render(&mut self) -> Option<()> {
        ...
        render_state.front_face_pass.render(
            &render_state.front_face_render_buffer.view,
            None,
            &mut encoder,
        );
        render_state.back_face_pass.render(
            &render_state.back_face_render_buffer.view,
            None,
            &mut encoder,
        );
        render_state
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        self.render_state = Some(block_on(RenderState::new(
            window.clone(),
            self.render_configs.sample_count,
        )));
        ...
        match &event {
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                ...
                if event.state.is_pressed()
                    && let Code(KeyCode::Escape) = event.physical_key
                {
                    event_loop.exit();
                }
            }
